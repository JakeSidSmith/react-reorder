  ...
  var getReorderable = function (React) {
    ...
    return React.createClass({
      ...
      handleTouchScrolling: function (event) {
        var element = this.getDOMNode();
        var rect = element.getBoundingClientRect();
        var border = this.getBorders(element);

        // Distance scrolled this frame
        var scrollDistanceTop = this.state.lastPos.clientY - event.clientY;
        var scrollDistanceLeft = this.state.lastPos.clientX - event.clientX;

        // Get element height without border (as border is not part of scrollHeight)
        var elementHeight = rect.height - border.top - border.bottom;

        if (scrollDistanceTop > 0) {
          // Find a parent element that needs scrolling if this element doesn't
          while(element.parentNode && (element.scrollHeight === elementHeight ||
            element.scrollTop === element.scrollHeight - elementHeight)) {
            element = element.parentNode;
            if (element.parentNode === document.body.parentNode) {
              elementHeight = window.outerHeight;
            } else {
              rect = element.getBoundingClientRect();
              border = this.getBorders(element);
              elementHeight = rect.height - border.top - border.bottom;
            }
          }
        } else if (scrollDistanceTop < 0) {
          // Find a parent element that needs scrolling if this element doesn't
          while(element.parentNode && (element.scrollHeight === elementHeight ||
            element.scrollTop === 0)) {
            element = element.parentNode;
        ...
        if (scrollDistanceTop !== 0) {
          ...
          if (element.scrollHeight > elementHeight) {
            // Handle scrolling
            element.scrollTop = element.scrollTop + scrollDistanceTop;
          }

          // If scrollable horizontally
          if (element.scrollWidth > rect.width) {
            // Handle scrolling
            element.scrollLeft = element.scrollLeft + scrollDistanceLeft;
          }
        }
