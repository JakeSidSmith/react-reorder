  ...
  var getReorderable = function (React) {
    ...
    return React.createClass({
      ...
      dragScrollX: function () {
        ...
        var distanceInArea;
        if (this.state.pointer.clientX < rect.left + this.constants.SCROLL_AREA) {
          distanceInArea = Math.min((rect.left + this.constants.SCROLL_AREA) - this.state.pointer.clientX, this.constants.SCROLL_AREA * 2);
          element.scrollLeft -= distanceInArea / this.constants.SCROLL_MULTIPLIER;
        } else if (this.state.pointer.clientX > rect.right - this.constants.SCROLL_AREA) {
          distanceInArea = Math.min(this.state.pointer.clientX - (rect.right - this.constants.SCROLL_AREA), this.constants.SCROLL_AREA * 2);
          element.scrollLeft += distanceInArea / this.constants.SCROLL_MULTIPLIER;
        }
      },
