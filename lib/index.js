    ...
    return React.createClass({
      ...
      autoScroll: function () {
        ...
        if (this.props.lock !== 'horizontal') {
          if (this.mouseOffset.clientX <= rect.left + scrollAreaX) {
            var scrollMultiplier = Math.min(Math.abs(rect.left + scrollAreaX - this.mouseOffset.clientX), scrollAreaX) / scrollAreaX;

            this.rootNode.scrollLeft = this.rootNode.scrollLeft - scrollMultiplier * CONSTANTS.SCROLL_SPEED;
          }

          if (this.mouseOffset.clientX >= rect.right - scrollAreaX) {
            var scrollMultiplier = Math.min(Math.abs(rect.right - scrollAreaX - this.mouseOffset.clientX), scrollAreaX) / scrollAreaX;
        ...
        if (this.props.lock !== 'vertical') {
          if (scrollTop > 0 && this.mouseOffset.clientY <= rect.top + scrollAreaY) {
            var scrollMultiplier = Math.min(Math.abs(rect.top + scrollAreaY - this.mouseOffset.clientY), scrollAreaY) / scrollAreaY;
          ...
          if (scrollTop < scrollHeight - rect.height && this.mouseOffset.clientY >= rect.bottom - scrollAreaY) {
            var scrollMultiplier = Math.min(Math.abs(rect.bottom - scrollAreaY - this.mouseOffset.clientY), scrollAreaY) / scrollAreaY;
