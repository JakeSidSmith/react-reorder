  ...
  function getReorderComponent (React, ReactDOM) {
    return React.createClass({
      ...
      autoScroll: function () {
        ...
        var scrollArea = Math.min(rect.height / 3, CONSTANTS.SCROLL_AREA_MAX);

        if (this.props.lock !== 'horizontal') {
          if (this.mouseOffset.clientX <= rect.left + scrollArea) {
            var scrollMultiplier = Math.min(Math.abs(rect.left + scrollArea - this.mouseOffset.clientX), scrollArea) / scrollArea;

            this.rootNode.scrollLeft = this.rootNode.scrollLeft - scrollMultiplier * CONSTANTS.SCROLL_SPEED;
          }

          if (this.mouseOffset.clientX >= rect.right - scrollArea) {
            var scrollMultiplier = Math.min(Math.abs(rect.right - scrollArea - this.mouseOffset.clientX), scrollArea) / scrollArea;
        ...
        if (this.props.lock !== 'vertical') {
          if (scrollTop > 0 && this.mouseOffset.clientY <= rect.top + scrollArea) {
            var scrollMultiplier = Math.min(Math.abs(rect.top + scrollArea - this.mouseOffset.clientY), scrollArea) / scrollArea;

            this.rootNode.scrollTop = this.rootNode.scrollTop - scrollMultiplier * CONSTANTS.SCROLL_SPEED;
          }

          if (scrollTop < scrollHeight - rect.height && this.mouseOffset.clientY >= rect.bottom - scrollArea) {
            var scrollMultiplier = Math.min(Math.abs(rect.bottom - scrollArea - this.mouseOffset.clientY), scrollArea) / scrollArea;
